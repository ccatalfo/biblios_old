...
function Pazpar2() {
	...
	this.getShowUrl = function getShowUrl(o) {
		...
		var showUrl = this.url + '?command=show&session=' + this.sessionid;
		if(o.start) {
			showUrl += '&start=' + o.start;
		}
		if(o.num) {
			showUrl += '&num=' + o.num;
		}
		if(o.block) {
			showUrl += '&block=' + o.block;
		}
		if(o.sort) {
			showUrl += '&sort=' + o.sort;
		}
	...
	this.getRecordMetadata = function getRecordMetaData(o) {
		if( this != self ) {
			return getRecordMetaData.apply(self, arguments);
		}
		var recid = o.recid;
		var successCallback = this.handleRecordMetaData;
		if( o.success ) {
	...
	this.getRecordMarc = function getRecordMarc(o) {
		...
		var successCallback = this.handleGetRecordMarc;
		if( o.success ) {
			successCallback = o.success;
		}
		$.ajax({
