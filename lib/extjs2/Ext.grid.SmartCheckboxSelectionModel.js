...
Ext.grid.SmartCheckboxSelectionModel = Ext.extend(Ext.grid.RowSelectionModel, {
	...
	getChecked: function(){
		...
		var count = this.grid.store.getCount();
        for(var i = 0, len = count; i < len; i++){
            var isChecked = this.grid.store.getAt(i).data[this.dataIndex];
            if(isChecked){
                checked.push(this.grid.store.getAt(i));
            }
        }
		return checked;
	},
	checkAllInStore: function() {
		var count = this.grid.store.getCount();
        for(var i = 0, len = count; i < len; i++){
            // Check
            this.grid.store.getAt(i).set(this.dataIndex, true);
        }
	},
    /**
