...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
  ...
  createGrid : function()
  {
    ...
    var cm = new Ext.grid.ColumnModel([
      ...
      bbar : new Ext.Toolbar({
        listeners: {
            beforerender: function(bbar) {
                bbar.autoCreate.html = '<table cellspacing="0"><tr></tr></table>';
            }   
        }
       } )
    });
  ...
  {
      if(bibliosdebug) {
	  console.debug('upload dlg: ' + data.response + ' ' + data.response.success);
	  console.info(data);
      }
